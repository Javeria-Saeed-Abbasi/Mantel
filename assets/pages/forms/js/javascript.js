...
$(document).ready(function () {
  ...
  $(add_button).click(function (e) {
    ...
    if (x < max_fields) {
      ...
      x++; //text box increment
      $(wrapper).append(
        '<div class="mb-3 mt-3 d-flex justify-content-center" ><input type="date" class="form-control py-3" name="multiple-dates"/>&nbsp;<a href="javascript:void(0);" class="btn btn-danger remove_field text-center mt-3"><i class="fa fa-minus"></i></a></div>'
      ); //add input box
    }
  });
...
$("#sibling").change((e) => {
  ...
  if (
    ...
    for (let index = 1; index <= e.target.value; index++) {
      console.log("hello");
      $(".more-siblings-wrap").append(` <div class="more-siblings-fields">
      <hr/>
      <div class="mb-4 form-group">
...
<select class="form-select py-3" id="" name="rel-w-sibling">
  ...
  <option value="">Poor</option>
      ...
      <button class="btn bg-danger remove_fields text-white" onclick="removeFunc(event)"> Remove </button>
...
// $("#sibling").change((e) => {
//   console.log(e.target.value);
//   let a = $(e.target.parentNode.parentNode).find(".sib");
//   let b = $(e.target.parentNode.parentNode).find("#other-sib")
